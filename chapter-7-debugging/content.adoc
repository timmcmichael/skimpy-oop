...
Did I make the mistake at the start of the program when I calculated the answer, or is the mistake at the very end where I output it? 
...
Putting code into production (releasing it in software that people actually use) without understanding it well enough to rigorously test it is a recipe for disaster.
...
Online tools like ChatGPT and Microsoft's Copilot can help us generate code without any specialized software. 
...
The user interface provides a chat window where we can describe what we want the code to do, and Copilot will generate code for us.
We can choose to accept that suggestions, or we can refine our prompt to get a different suggestion.

Additionally, Copilot can generate code as we type.
It will analyze the code we've already written and suggest the next line of code.
We can accept that suggestion, or we can ignore it and keep typing.
It essentially acts as a very advanced code completion tool--or like the "autocomplete" feature on our phone's keyboard, but for code.

At this time, GitHub Copilot requires a subscription, but we can use it for free for a limited number of lines of code; and students get an expanded free tier.
