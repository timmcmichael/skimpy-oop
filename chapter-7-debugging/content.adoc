...
* Learn some strategies for debugging our code
* Use the debugging tools available in our IDE
...
Did we make the mistake at the start of the program when we calculated the answer, or is the mistake at the very end where we output it? 
Or did we do something in the middle that accidentally changed the result?
...
Even if we're not using Visual Studio Code, the concepts are the same, so we should be able to apply what we learn here to our IDE of choice.
...
IMPORTANT: The first Lab Assignment in Canvas can be completed using what we've covered to this point. We might choose to complete that work now, then move onto the next section--which we'll need for the second Lab Assignment.
...
I would like to think it goes without saying, but based on the number of students who deny this, I guess I need to say it: using AI to generate code that we submit as our own is cheating.
It is no different than copying someone else's code and submitting it as our own.
...
But the impact this technology has had--and will continue to have--on the field of software development is undeniable, and I would be doing us a disservice if we didn't learn a little about it.
...
* *Code completion:* Many IDEs already have code completion features that suggest code as we type.
Generative AI can take this a step further by suggesting entire lines of code, or even entire methods.
* *Code generation:* Generative AI can generate code based on a description of what we want the code to do.
...
Coders who rely heavily on artificial intelligence tools to solve problems may be able to pass themselves off as more skilled
