...
#This section is not quite finished, but in the meantime this content is covered in section 4.2 (page 117) through 4.5 in the textbook.#
...
Because of that convention, we also call these methods *getters*.
...
The whole point of a getter is to allow other classes to be able to read the value of a field.
If other classes don't need to know the value, we just don't write a getter for that field.
