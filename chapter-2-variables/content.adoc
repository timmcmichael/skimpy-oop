...
The `Scanner` class includes a variety methods for working with input "streams" (including input sources other than a keyboard), but the ones you'll need for our work are:
...
// TODO: finish Scanner reference

WARNING: In case you didn't catch this in the video, there's a weird quirk that happens when you get numeric input from a user and then ask for String input using `next()` or `nextLine()`. If your program seems to skip that String input line, review that part of the video!
