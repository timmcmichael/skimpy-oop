...
Single-letter variable names, like `x` for a number, is fine in your math class, but is generally *not* okay when you're coding.
The expectation in my course is that your code will align with those conventions--because that's what industry expects--and you will lose points if you don't.
...
A `byte` has plenty of room (up to 128) to store even the oldest person's age, and it uses a fraction of the memory.
...
Second, these days even the most pathetic computer has waaaaaay more RAM than is needed by even the most complex program a beginner will write, so we don't need to worry about it.
...
NOTE: I pronouce it "char" like "chart" without the "t" and firmly believe that only a sociopath would say it like the word "care".
...
If we want to put a collective of characters together to make words and sentences, we need multiple ``char``s grouped into a single variable.
That data type is called `String`, because it strings together a bunch of ``char``s, light a string of holiday lights.
...
NOTE: Always remember that, in Java, single quotes means a `char` and double quotes means a `String`. It's easy to get them mixed up--especially if you use Python, where they are interchangeable--but your code won't compile if you mix them up.
