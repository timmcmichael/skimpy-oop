...
Java provides two basic ways to output text to the console: `System.out.print()` and `System.out.println()` statements.
`print()` outputs whatever is in the parentheses, and for now we'll put that text in quotation marks:
...
After the `print()` statement executes, the cursor is still sitting right after the _*r*_ in *_Jagger_*, so when the next line of code outputs _*you can't always*_, that text just gets jammed onto the end. 
Notice that it doesn't even add a space; if we want a space there, we have to include it in our parentheses.
...
NOTE: Source code examples from this chapter are available on  https://github.com/timmcmichael/EMCCTimFiles/tree/4bf0da6df6f4fe3e3a0ccd477b4455df400cffb6/OOP%20with%20Java%20(CIS150AB)/01%20Computers%20and%20Coding[GitHub].
...
It it referring to an old Rolling Stones song.
...
Coding is all about trial and error, and programmers spend a lot of time going writing code in different ways until they get it working the way they want.
