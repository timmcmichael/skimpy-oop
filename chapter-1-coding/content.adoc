...
The coach tells the QB "22" and the QB knows which play to run, because they're agreed on what instruction (play) the number 22 means--and there are actually football teams that do exactly this.
...
Okay, back to those commands that a computer can execute super fast.
Each command, or *instruction* (like football plays), is represented by a number--a binary number, since that's the number system the computer uses.
The collection of these numbered instructions the computer understands is called *machine language*.
...
* link:source/HelloWorld.java[HelloWorld.java]
****
