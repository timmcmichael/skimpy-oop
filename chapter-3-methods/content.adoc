...
But if you put that calculation in a method and use that method ten times, you can just update the calculation inside the method and your changes will automatically be used all ten times.
You'll see more advantages to methods as you learn more about prgramming.
...
To output text, we've been calling the `println()` or `print()` methods.
...
[source,java]
----
public static displayCopyright() {
    System.out.println("(C) 2025 by Tim McMichael. All rights reserved.")
}
----

- The `public` keyword is an _access modifier_. Defining the method as `public` makes it possible to call the method from anyway. More on this later, and for now all of your methods will be `static`.
- The `static` keyword basically means that the method belongs to the class and not an object. Again, you'll lear about this soon and for now your methods will be `static`.
- `displayCopyright` is the identifier for the method, in the same way variables have identifiers. Like variable identifiers, use camelCase to make the name readable--and make the name descriptive. The convention is that the name of a method should describe what it does; usually, that means the identifier is a verb.
